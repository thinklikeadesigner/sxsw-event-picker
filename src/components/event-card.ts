...
export function renderEventCard(event: SXSWEvent, starred: boolean, conflictCount: number): string {
  return `
    <div class="event-card ${starred ? 'starred' : ''} ${conflictCount > 0 ? 'has-conflict' : ''}"
      ...
      ${event.location ? `<div class="event-location">\uD83D\uDCCD ${event.location} <a class="map-link" href="https://maps.google.com/?q=${encodeURIComponent(event.location)}" target="_blank">Map</a></div>` : ''}
